...
## 9-terdecies. Autocomplétion des lieux (carte) + phrase de login générique (2026-07-15)
> Tout dans `index.html`.
- **Autocomplétion Nominatim** sur TOUS les champs de saisie de lieu de la carte (panneau « 📍 lieux à renseigner » ET champ « ✏️ Corriger » d'un lieu supposé). Les 2 `<input>` reçoivent `class="map-place-input" data-id="<idFiche>" autocomplete="off"`. Module JS global (inséré avant `_mapConfirmGuess`) : objet d'état `_placeAC{box,input,items,idx,timer,seq,lastReq,ctrl}` + fonctions `_placeACClose`, `_placeLabel(it)` (extrait `address.postcode`/ville/pays → `{main,sub,full}`), `_placeACShow`, `_placeACPick(i)`, `_placeACHi`, `_placeACPosition`, `_placeACFetch(input)`, `_initPlaceAC()` (appelé une fois au chargement, garde `window._placeACInit`).
  - Écoute **déléguée** sur `document` (`input`/`keydown` capture/`click`) : **debounce ~400 ms**, aucune requête < 3 car., espacement **≤ 1 req/s** (`_placeAC.lastReq`), annulation des requêtes obsolètes (`seq` + `AbortController`). Requête `https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&limit=5&q=…`.
  - Menu déroulant `.place-ac` (div `position:fixed` **appended au body**, `z-index:400` > `.modal-bg` 60) → ne déborde jamais (largeur clampée à `innerWidth-left-8`, `scroll-into-view`). Clavier ↑/↓ + Entrée (sélectionne la surbrillance, sinon fallback `_mapQuickGeo` = géocodage du texte), Échap/clic extérieur ferme.
  - **Clic/sélection** : utilise directement `it.lat`/`it.lon` (PAS de re-géocodage) → `f.geo={lat,lng,q:label,manual:true,mq:label}` + `save()` + `toast` + `_refreshMap()`. Fallback réseau KO = pas de menu (jamais d'erreur).
  - CSS `.place-ac`/`.place-ac-item`/`.pa-main`/`.pa-sub`/`.place-ac-empty` (sombre, `--panel2`/`--line`/`--muted`).
- **Phrase de login générique** (`#unlockOverlay`) : « Entre le code d'accès pour afficher les données de David & Iris. » → **« Entre ton code d'accès pour ouvrir ton espace Guso Facile. »** (logique de déverrouillage inchangée).
- **Testé** (port 8000, `pushRemote` neutralisé, espace artiste, David) : dans « lieux à renseigner », taper `10 rue de la roquette paris` → suggestions **« Rue de la Roquette, Paris / 75011 · Paris · France »** (code postal visible) ✅ ; taper `lyon` puis **clic** sur « Lyon, France » → `f.geo={lat:45.7578137,lng:4.8320114,manual:true,q:"Lyon, France"}` (lat/lng = ceux de la suggestion, marqueur posé, menu refermé) ✅ ; **375 px** : menu `left:50 → right:290` (vw 375) **sans débordement** ✅ ; phrase de login = nouvelle version ✅ ; page **rechargée** + données de test purgées, cloud NON pollué.

## 9-quaterdecies. Module « 🐛 Signaler un bug / Feedback » (Supabase) (2026-07-15)
> Tout dans `index.html`. Module 100 % indépendant du reste (n'utilise ni Firestore ni l'état de l'app) : si Supabase est injoignable, l'app reste pleinement fonctionnelle.
- **Bouton** « 🐛 Signaler un bug » dans `#hdrActions` (après « ❓ Guide ») → visible desktop + menu mobile ☰.
- **Modale `#bugBg`** (`.modal-bg`/`.modal`, croix `.bo-x`, plein écran mobile, `z-index:85`). Champs : Type (`bug_kind` → bug/feature/question), Catégorie (`bug_category` → dpae/guso/facture/dates/carte/tournee/sync/mobile/affichage/compte/autre), Description obligatoire (`bug_body`), « Est-ce que ça te bloque ? » (`bug_blocks` → oui/un_peu/non), Gravité optionnelle 0-3 (`bug_severity`), Nom + email optionnels (`bug_name` pré-rempli avec `fullName(currentPerson)`, `bug_email`).
...
## 9-quindecies. Prototype beta multi-comptes `beta.html` + console admin (2026-07-15)
> Phase 6 (bac à sable) : page publique SÉPARÉE `beta.html` (920 lignes, tout inline, supabase-js@2 CDN UMD) → `https://david-lesage.github.io/intermittence-david-iris/beta.html`. NE touche NI `index.html` NI Firestore/données live. Parle uniquement au sandbox `wqhwfqasoyyeprggjxet`.
- **Backend ajouté au sandbox** :
  - Migration `admin_role_and_beta_gate` : défauts sûrs (`entitlements.beta=false`, `status='none'` → un inscrit n'a PAS l'accès), `handle_new_user` crée profile + entitlements avec **bootstrap super-admin `contact@lesagedavid.fr`** (→ role=admin, status=active, beta=true dès son inscription), fonction `is_admin()` (SECURITY DEFINER, pattern Handpan), policies admin (SELECT sur profiles/entitlements/feedback + UPDATE feedback), revoke execute de `handle_new_user` (durcit 2 advisors).
  - **Edge Function `admin-users`** (verify_jwt, service_role côté serveur, copie adaptée de Handpan Studio) : actions `list` (tous les comptes : auth.users paginé + entitlements + profiles), `setBeta` (offrir/retirer — tous admins), `setRole` (nommer/retirer admin — **super-admin uniquement**, impossible de rétrograder le super-admin), `invite` (inviteUserByEmail + beta d'office, `redirectTo` = APP_URL défaut GitHub Pages beta.html). CORS multi-origines via env `ALLOWED_ORIGIN` (défaut `*`). Aucune suppression de compte exposée. Écritures entitlements = UNIQUEMENT via cette EF (aucune policy write client).
- **`beta.html`** (thème sombre repris des variables d'index.html) : login **mot de passe + lien magique** (`signInWithOtp`), inscription (nom + choix 🎤 Artiste/🏛️ Structure + `options.data.full_name`), mot de passe oublié ; écran « définis ton mot de passe » (capture `type=invite`/`type=recovery` dans le hash AVANT `detectSessionInUrl`) ; **garde beta** (accès = admin || beta || status beta/active, sinon écran « accès sur invitation ») ; espace connecté 4 onglets : 👤 Profil (édition `profiles`), 📅 Mes dates (CRUD `gig_dates` + rattachement projet, dates des co-membres en lecture seule « via projet X »), 🤝 Projets (créer, inviter par email, accepter/refuser une invitation → self-insert `project_members`), 🛡️ **Admin** (visible si role=admin : tableau des comptes, recherche, 🎁 Offrir/🚫 Retirer l'accès, ⭐ Nommer/Retirer admin si super-admin, ✉️ Inviter par email). Pied : « 🧪 Prototype beta — bac à sable » + lien app principale.
- **Vérifié** (agent + re-vérif orchestrateur, serveur local + Browser pane) : console 0 erreur, login/inscription rendus desktop ET **375 px sans débordement** (`scrollWidth=375`), signup réel → « vérifie tes emails » (confirmation email ACTIVE sur le sandbox), garde beta OK. Livré commit `f3bdca7`.
- ⚠️ **RESTE À FAIRE côté dashboard Supabase (manuel, MCP ne peut pas)** : Auth → URL Configuration → ajouter `https://david-lesage.github.io/intermittence-david-iris/beta.html` en Site URL/Redirect URLs (sinon liens magiques/invitations → localhost:3000). Puis **David s'inscrit avec contact@lesagedavid.fr** → devient super-admin automatiquement.
- Limites connues : flux connecté complet (profil/dates/projets) non exercé bout-en-bout faute de compte confirmé — à tester avec le vrai compte de David ; `full_name`/rôle choisis à l'inscription écrits dans profiles seulement si session immédiate (avec confirmation email → à ajuster dans l'onglet Profil) ; nom du projet d'une invitation reçue = fallback « un projet » si RLS ne l'expose pas encore.
- Note env : `.claude/launch.json` racine `~/CLAUDE/` a gagné une config `guso-static` (python http.server 8642 --directory "GUSO FACILE") pour les previews.

## 9-sexdecies. GRANDE BASCULE multi-comptes (2026-07-17/18) — état au 18/07
> Décision David : l'app complète passe au multi-comptes Supabase pour TOUT LE MONDE (auth+RLS remplacent le chiffrement `gusofacile` — nécessaire pour la Phase 5 emails). Iris consent explicitement au partage de ses données (accès admin Yannick inclus, décision David).
- **Sauvegarde live VÉRIFIÉE avant tout** : `~/CLAUDE/GUSO-FACILE-BACKUPS/2026-07-17/` (blob Firestore chiffré restaurable + state clair 103 Ko ; 64 fiches, périodes en cours **David 521 h / Iris 412 h**, totaux toutes-périodes de contrôle 1081/908). **Firestore d'origine INTACT** (filet de sécurité). Script : scratchpad `backup-guso.mjs` (lecture seule, REST + PBKDF2/AES-GCM).
- **Vercel** : projet `guso-facile`, URL courte **https://guso-facile.vercel.app** (alias à refaire pointer après chaque `npx vercel --prod` !). `.vercelignore` exclut le handoff. GitHub Pages reste en parallèle.
- **Backend sandbox (`wqhwfqasoyyeprggjxet`)** — migrations : `account_requests` (demandes d'accès publiques, INSERT anon anti-escalade testé), `spaces_state` (table `spaces` = state JSONB entier par compte + `space_members` + `is_space_member`), `structure_affiliations` (tables `structures`/`structure_artists` + `manages_space`/`owns_structure`), `affiliation_visibility(+v2)` (colonne `visibility` full/partner/minimal + RPC filtrées). **Espace « David & Iris »** id `ff996088-4ccd-4510-b30d-b2938542b2ce`, owner contact@lesagedavid.fr, Iris membre — 64 fiches migrées, sommes vérifiées EXACTES (521/412 période en cours).
- **Edge Function `admin-users` v5** : list/setBeta/setRole/invite + **listRequests/approveRequest/rejectRequest** (demandes) + **listStructures/affiliate/unaffiliate/setVisibility** (structures). Toutes les écritures entitlements/affiliations passent par elle.
- **Confidentialité graduée** (demande David, ex. HARMONIE/Cédric Moulié) : par affiliation `visibility` = 🤝 full (accès direct blob, cas Des Sons et Des Liens) / 🧾 partner (défaut : RPC `structure_read_space` — fiches circuit GUSO complètes, fiches studio+`conf='montants'` expurgées des MONTANTS seulement (brut/cond/factures/notes ; lieu+orga+cachets visibles), `conf='privee'` = invisible mais comptée dans les totaux, profils sans secu/iban/rib) / 🔢 minimal (totaux seuls). Écriture partner via `structure_update_fiche` (fiche par fiche). L'artiste voit/règle son `visibility` (trigger de garde : il ne peut changer QUE ça). TESTS RLS réels passés (fuite montants=0, accès direct refusé en partner, révocation immédiate).
- **Page de vente** : formulaire « ✨ Demander un accès » (prénom/nom/email/tél/artiste-structure/message) → `account_requests` (201/409 testés) ; CTA « Se connecter ». DÉPLOYÉ (GitHub + Vercel).
- **`index.html` UNIFIÉ (4247 lignes) — PAS ENCORE DÉPLOYÉ** : Firebase+chiffrement supprimés (ENC_SEED/CRYPTO_KEY=0 résidu), connexion Supabase (mdp+magic link+invite/recovery+oubli), persistance `spaces` (debounce 800 ms, localStorage en cache), console admin 2 onglets (📥 Demandes badge+approuver/refuser · 👥 Comptes), Déconnexion. Vérifié : syntaxe OK, 0 erreur console, jauge **521/507** avec les 64 vraies fiches injectées, 375 px OK, deep-link rejoué après login. **BLOQUANT : David doit tester la connexion réelle (localhost:8642/index.html, compte contact@lesagedavid.fr) avant déploiement** — le transport réel (RLS spaces via session navigateur) n'a pas pu être exercé sans son mot de passe.
- **Comptes** : David super-admin ✅ confirmé · Iris admin (invitée, email pas encore confirmé) · Yannick admin (décision David : superviseur marketing, voit tout) · compte de test supprimé. `beta.html` devient redondant après déploiement (à décommissionner/rediriger).
- **⚠️ À durcir avant commercialisation** : (1) « admin » voit tous les espaces (policy `is_admin()` sur spaces) — OK entre proches, pas avec de vrais clients ; (2) heuristique fiche↔structure = `type<>'studio'` (v1) — le vrai fix = `structure_id` explicite par fiche ; (3) totaux minimal `cachets*12+repet` sans `repetIris` ; (4) INSERT anon feedback (spam) ; (5) revoir Auth leaked-password-protection. Détails/limites dans les comptes-rendus d'agents.
- **File d'attente immédiate** : ① test connexion David → déploiement index.html unifié ② UI artiste « régler la confidentialité par structure » + `f.conf` à la contractualisation ③ affilier réellement Des Sons et Des Liens (compte Christophe) ④ UI structure (vue via RPC filtrée) ⑤ décommissionner beta.html ⑥ nettoyer les 2 lignes « TEST agent » de la table feedback (module bug, 15/07).

## 9-septdecies. Vue Groupe + Besoin d'aide + refonte vente + blog (2026-07-18, suite)
> Journée de la grande bascule, 2e partie. TOUT déployé sur guso-facile.vercel.app + GitHub Pages.
- **App DÉPLOYÉE multi-comptes** (test réel David : 521 h ✅ affichées après connexion). Firestore d'origine intact (filet).
- **🎸 Vue Groupe** (demande David : solidarité de groupe, consentement unanime) : backend `music_groups`/`music_group_members` (consent invited/accepted/declined/left, unique (group,user,person)), RPC `group_dashboard(gid)` (membres triés rouge→days_left, vigilance auto, sources ce_groupe/autres/perso via `f.groupe`, JAMAIS de montants, périodes individuelles par anniv) + `structure_dashboard()` (idem pour les structures affiliées, filtré par visibility). Seuils v1 : rouge = rythme>60 h/mois OU (restant>0 ET J<60) ; orange >30. UI : bouton 🎸 Groupe, création + **rejoindre par CODE** (self-insert RLS vérifié OK ; invitation email = plus tard), consentement explicite (texte clair sur ce qui est partagé), dashboard cartes membres 🔴🟡🟢 + jauge + J-X + sources, quitter. Champ « 🎸 Groupe » optionnel sur les dates (datalist) → alimente sources.
- **🤗 J'ai besoin d'aide** : table `help_requests` (kind prochain_pas/retard_heures/blocage_admin/doute_perso, urgency, UN destinataire groupe/structure/personne, RLS auteur+destinataires, anon refusé — testé). UI : questionnaire 3 étapes, **conseil d'abord** (vrai prochain pas depuis renderTodoNow, vrais chiffres de retard), partage = choix explicite (doute_perso : jamais pré-coché, message sobre), bandeau réception + « 👍 Je m'en occupe » (acked).
- **Page de vente refondue** : 4 univers (🎯 droits / 🎪 tournée / 🏢 structure / 🫂 cercle Bientôt) = 22 fonctionnalités réelles recensées + chips « Et aussi » + liens blog. Sections entraide + « J'ai besoin d'aide » (badges Bientôt honnêtes partout).
- **📚 Blog** : `blog.html` + `blog/` 8 articles cas d'usage scénarisés (507h/DPAE/pointage FT/tournée/bon plan/duo/structure/studio), personnages Léa/Marco/Sophie, encadrés « 💡 Dans l'app », cross-links, fil rouge « moins d'énergie dans l'admin, plus dans la musique ». 40 liens internes vérifiés 200.
- **À tester en réel (avec David+Iris)** : création groupe Solune par code, consentement, dashboard réel, aide inter-comptes, destination structure (aucune affiliation active en base pour l'instant). Restes connus : invitation groupe par email, exposition UI du réglage visibility par l'artiste, `f.conf` par fiche à la contractualisation, sources structure (ce_groupe=0 par design), timezone anniv (±1 j), person_key=currentPerson à la création.
...
- Prototype beta multi-comptes : beta.html (auth mot de passe + lien magique, garde beta, profil, dates cloisonnées, projets partagés, console admin) + migration admin + Edge Function admin-users sur le sandbox
